...
execute if score @s tick0 matches 302 as @e[type=#gd_main:companion,tag=frostguard,limit=1,sort=nearest] at @s run function gd_companion:select/type/frost_golem/ice_summon

execute if score @s tick0 matches 3..71 as @e[tag=enemy] at @s if entity @e[type=armor_stand,tag=iceblock,distance=..2] run function gd_companion:select/type/frost_golem/ice

...
execute if predicate gd_main:time_check_gauge run particle minecraft:spore_blossom_air ~ ~1.5 ~ .3 .8 .3 0.001 5
...
execute if score @s tick0 matches 302 as @e[type=#gd_main:companion,tag=poisonguard,limit=1,sort=nearest] at @s run function gd_companion:select/type/poison_golem/attack_poison_summon

execute if score @s tick0 matches 3..71 as @e[tag=enemy] at @s if entity @e[type=armor_stand,tag=poisonblock,distance=..2] run function gd_companion:select/type/poison_golem/attack_poison

...
function gd_enchantment:player
execute if score @s used.carrot_on_a_stick matches 1.. run function gd_main:exe/carrot_on_a_stick
...
# execute as @s[scores={talked_to_sword_equip.cooldown=0..}] run scoreboard players remove @s talked_to_sword_equip.cooldown 1
# execute as @s[scores={talked_to_sword.cooldown=0..}] run scoreboard players remove @s talked_to_sword.cooldown 1
# execute as @s[scores={talked_to_villager.cooldown=0..}] run scoreboard players remove @s talked_to_villager.cooldown 1
# execute as @s[scores={node.cooldown=0..}] run scoreboard players remove @s node.cooldown 1
...
# execute if score @s rune.companionCount matches 3 as @e[tag=companion,limit=1,sort=random] if score @s UUID = @p UUID run effect give @s minecraft:strength 12 0 false
# execute if score @s rune.block.counteraction_range matches 10 run effect give @s minecraft:haste 1 0 true
...
# execute if score @s rune.safe_fall_distance matches 10 if predicate gd_main:on_slime run effect give @s minecraft:jump_boost 1 15 true

# scoreboard players enable @s settings
# execute if score @s settings matches 1.. run function gd_main:check/settings

# execute unless score @s enchantment.knife_pouch.count >= @s enchantment.knife_pouch_MAX run scoreboard players add @s enchantment.knife_pouch_recharge 1
...
# execute if items entity @s weapon.mainhand minecraft:sentry_armor_trim_smithing_template[minecraft:custom_data={range1:1b}] run function gd_main:misc/rune/block.counteraction_range/display
...
# execute if score @s companionCount > @s rune.companionCount as @e[tag=!healer,tag=companion,limit=1,sort=furthest] as @s if score @s UUID = @p UUID run kill @s

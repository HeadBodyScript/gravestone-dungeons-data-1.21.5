...
execute at @s if score @s bossTick matches 1 unless score @s boss.grimgar_minion.count >= grimgar.minion.max INT as @e[type=minecraft:marker,distance=..32,tag=marker.minion,limit=3,sort=random] at @s run function gd_boss:grimgar/other/minion

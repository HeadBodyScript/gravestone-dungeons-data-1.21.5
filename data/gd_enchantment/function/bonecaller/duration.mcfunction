scoreboard players remove @s enchantment.bonecaller.duration 1
...
execute if entity @s[scores={enchantment.bonecaller.duration=0}] run function gd_enchantment:bonecaller/other/reset
execute at @s as @e[distance=..2] unless score @s UUID = @e[type=armor_stand,tag=projectile_bonecaller,limit=1,sort=nearest] UUID run function gd_enchantment:bonecaller/attack